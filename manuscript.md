...
<small><em>
This manuscript
([permalink](https://fmaguire.github.io/mag_sim_paper/v/ed642d1265ab160264b8d3fc1b02b051ba881b8b/))
was automatically generated
from [fmaguire/mag_sim_paper@ed642d1](https://github.com/fmaguire/mag_sim_paper/tree/ed642d1265ab160264b8d3fc1b02b051ba881b8b)
on February 3, 2020.
</em></small>
...
Metagenomics, the sequencing of fragments of DNA from within an environmental sample, is widely used for characterising viral and microbial communities [@8PLOeAH6; @7RV1Ygsv]. 
...
These approaches have been instrumental in developing our understanding of the distribution and evolutionary history of AMR genes [@rwhLEYRY; @QSe5BqFk; @2xaXclNM], as well as tracking pathogen outbreaks [@khJQfjDf].

While long-read DNA sequencing technology (e.g., Oxford Nanopore [@19dz2SKIf], PacBio [@QbXTukk0]) is now being used for metagenomic sequencing [@U4vhNZoB; @1759XyDVi],  high-throughput sequencing of relatively short reads (150-250bp) in platforms such as the Illumina MiSeq currently dominate metagenomic analyses.
...
Sequence assembly is often used to genereate information about genomic context [@F7RexqdF]. 
de Bruijn graph-based assemblers have been developed to handle the particular challenges of this type of data including metaSPAdes [@KP5SjPXN] , IDBA-UD [@a4mT7fuU], and megahit [@1EUV0Ejkr].
...
This compounded data loss means that only a relatively small proportion of reads are successfully assembled and binned in large complex metagenome datasets, for example, 24.2-36.4% of reads from permafrost [@buqrbdBh] and soil metagenomes [@d5Hh0941]. 
...
The Critical Assessment of Metagenome Interpretation (CAMI) challenge's (https://data.cami-challenge.org/) Assessment of Metagenome BinnERs (AMBER) [@Y8sHlHi] assesses the global completeness and purity of recovered MAGs across methods.
...
GIs can arise following the integation of MGEs, such as integrons, transposons, integrative and conjugative elements (ICEs) and prophages (integrated phages) [@DET3tBYj; @1Af4oXwEX]; they disproportionately encode virulence factors [@LxGqo7iq] and are a major mechanism of LGT of AMR genes [@x7HhCKyS; @17U91060Y]. 
GIs often have different nucleotide composition compared to the rest of the genome [@DET3tBYj].
...
In accordance with the recommendation in the CAMI challenge [@lsbnKJf8] the genomes were randomly assigned a relative abundance following a log-normal distribution (μ = 1, σ = 2).
...
MiSeq v3 250bp paired-end reads with a mean fragment length of 1000bp (standard deviation of 50bp) were then simulated using art_illumina (v2016.06.05) [@znONJtTo] resulting in a simulated metagenome of 31,174,411 read pairs.
...
For the reference genomes, as well as 12 sets of MAGs prodigal [@lX665mdh] was used to predict open reading frames (ORFs) using the default parameters. 
...
Virulence factors were predicted using the predicted ORFs and BLASTX 2.9.0+ [@nEsJGUWa] against the Virulence Factor Database (VFDB; obtained on Aug 26, 2019) with an e-value cut-off of 0.001 and a minimum identity of 90% [@pYB1SP5].
...
For each MAG set, we counted the total number of AMR/VF genes recovered in each assembly and each MAG and compared this to the number predicted in their assigned reference chromosome and plasmids.
...
We then explored the ability of different approaches to find open reading frames (ORFs) within MAGs.
...
As with AMR genes, the genomic context (chromosome, plasmid, GI) of a given VFs largely determined how well it was binned (Fig. @fig:VFGenePercentRecoveryStage).
...
With repetitive DNA sequence particularly difficult to correctly assemble from short reads [@UWOTvAMZ,@12zFifp5x].
